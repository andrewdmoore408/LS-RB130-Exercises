...
def map(collection)
  collection.each_with_object([]) { |index, mapped| mapped << yield(index) }
end
...
# Further exploration: since my implementation uses a method from Enumerable, it will work with Hashes and Sets. A test case is below
p map({foo: 1, bar: 2, baz: 3, quz: 4}) { |k, v| [k, v * 2] } == [[:foo, 2], [:bar, 4], [:baz, 6], [:quz, 8]]